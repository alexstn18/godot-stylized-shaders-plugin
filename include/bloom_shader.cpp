                                   ...
                                   RenderData *p_render_data)
{
        ...
        p_effect_callback_type == EFFECT_CALLBACK_TYPE_POST_TRANSPARENT)
    {
        ...
        if (buffers.is_valid() || !scene_data)
        {
            ...
            auto usage = RenderingDevice::TEXTURE_USAGE_SAMPLING_BIT | RenderingDevice::TEXTURE_USAGE_STORAGE_BIT | RenderingDevice::TEXTURE_USAGE_CAN_UPDATE_BIT | RenderingDevice::TEXTURE_USAGE_CAN_COPY_TO_BIT;
            if(!m_mip_finished) // run this only once, but i need size which is gotten from p_render_data..
            {
                ...
                for(int32_t i = 0; i < m_num_sampled_mips; ++i)
                {
                    ...
                    buffers->create_texture("Bloom", downsample_name, RenderingDevice::DATA_FORMAT_R16G16B16A16_SFLOAT, usage, RenderingDevice::TEXTURE_SAMPLES_1, m_mip_resolutions[i], 1, 1, true, false);
                    buffers->create_texture("Bloom", upsample_name, RenderingDevice::DATA_FORMAT_R16G16B16A16_SFLOAT, usage, RenderingDevice::TEXTURE_SAMPLES_1, m_mip_resolutions[i], 1, 1, true, false);
                }   
                m_mip_finished = true;
            }
            ...
            for (auto i = 0; i < view_count; ++i)
            {
                ...
                for(int32_t mip = m_num_sampled_mips - 1; mip >= 0; --mip)
                {
                    ...
                    {
                        src_texture = buffers->get_texture_slice("Bloom", "Downsample" + String::num(mip + 1, 0), i, 0, 1, 1);
                    }
                    else
                    {
                        src_texture = buffers->get_texture_slice("Bloom", "Upsample" + String::num(mip + 1, 0), i, 0, 1, 1);
                    }

                    auto downsample_texture = buffers->get_texture_slice("Bloom", "Downsample" + String::num(mip, 0), i, 0, 1, 1);
                    
                    auto dst_texture = buffers->get_texture_slice("Bloom", "Upsample" + String::num(mip, 0), i, 0, 1, 1);
