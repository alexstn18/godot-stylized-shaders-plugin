...
{
    ClassDB::bind_method(D_METHOD("set_effects", "effects"), &ManualArrayInspector::set_effects);
    ClassDB::bind_method(D_METHOD("get_effects"), &ManualArrayInspector::get_effects);
    ClassDB::bind_method(D_METHOD("_on_move_pressed", "from_index", "to_index"), &ManualArrayInspector::_on_move_pressed);

    ADD_SIGNAL(MethodInfo("order_changed", PropertyInfo(Variant::ARRAY, "effects")));
}
...
{
    if(to_index < 0 || to_index >= m_effects.size()) return;
